  ...
  constructor() {
    ...
    this.stopBtn.addEventListener('click', () => { 
      ...
      if(panelsRemain === 0) {         // 判定 //
        checkForMatchedAll();          // 判定 //
        checkForUnMatched();           // 判定 //
        checkForTwoPairMatched();      // 判定 //
        checkForTwoPairExtraSeven();   // reset shadow effect //
        checkForTwoPairExtraDiamond(); // reset shadow effect //
        checkForTwoPairExtraBar();     // reset shadow effect //
        panelsRemain = 3;      // reset //
        spinStopBtnEvent = false; 
        activeLight = false;
        applyCheckOut = false;
        betCount = false; /////////////////////////////////////////////////////// !!! 二度打ち防止
        // panels.forEach(stop => {  // * MY * reset //
        //   stop.stopBtn.classList.remove('js_inActive'); 
        //   });
...
// out of constructor -----------------------------------------------------------------------
  ...
  spin() {
    ...
    this.stopBtn.classList.remove('js_inActive'); // reset btn opacity // * MY * THIS OFF //
...
function checkForMatchedAll() {
  if(panels[2].matched(panels[1], panels[0])) {  // new //
    pointAdd_matchedAll();
    if((panels[0].img.src === panels[1].img.src) && panels[2].img.src.includes('diamond')) return; //////////////////////////////////////////////// !!!
        bet2x.classList.remove('js_bet2x-activeEffect'); // reset bets2x //
        bet2x.classList.remove('js_bet2x-textColorBright');
      bet5x.classList.remove('js_bet5x-activeEffect'); // reset bets5x //
...
function checkForTwoPairMatched() {
  if((panels[0].img.src === panels[2].img.src) 
  &&  panels[1].img.src.includes('diamond')) { // reset point add twice //
    return;
  }
  if((panels[0].img.src === panels[2].img.src) 
  &&  panels[1].img.src.includes('seven')) { // reset point add twice //
    return;
  }
  if((panels[0].img.src === panels[2].img.src) 
  &&  panels[1].img.src.includes('bar')) { // reset point add twice //
    return;
  }
  if((panels[0].img.src === panels[2].img.src) 
  && panels[2].img.src !== panels[1].img.src) { // original checkForTwoPairMatched //
    pointAdd_twoPair();
  }
}
...
const winPoint = document.querySelector('.win-point');
  const totalPoint = document.querySelector('.total-point');
    const lostPoint = document.querySelector('.lost-point');
...
function pointRate(arg) {
  if(panels[2].img.src.includes('seven')) {
    ...
    setTimeout(() => { // BIG SPIN // // BIG SPIN // // BIG SPIN //
      panels.forEach(panel => {
        ...
        // winPoint.textContent = 'BIG SPIN'; needs cancel somewhere /////////// !!!
        console.log(insertPoint);
      });
    }, 500); // BIG SPIN // // BIG SPIN // // BIG SPIN //
    // total += 3000 * arg; 
    // winPoint.textContent = 3000 * arg; 
    winPoint.classList.add('js_winRed'); 
  } else if(panels[2].img.src.includes('bar')) {
    total += 1500 * arg;
    ...
    total += 1000 * arg;
    winPoint.textContent = 1000 * arg;
    winPoint.classList.add('js_winRed'); 
  } else if(panels[2].img.src.includes('cherry')) {
    total += 500 * arg;
    winPoint.textContent = 500 * arg;
    winPoint.classList.add('js_winRed'); 
  } else if(panels[2].img.src.includes('bell')) {
    total += 300 * arg;
    winPoint.textContent = 300 * arg;
    winPoint.classList.add('js_winRed'); 
  }
  if(insertPoint.classList.contains('js_blank')) { // BIG SPIN READ TEST //
...
// two pair match and two pair match extraは ポイント加算成功してた --- !!!
...
// if(total > 10000) first setting
function betCounter() {
  console.log('bet counter read in function');
  if(betCount) return;
  if(total > 10000 && currentDept >= 10000 || (total > 0 && currentDept === 0)) {
    if(bet5x.classList.contains('js_bet5x-activeEffect') 
    && bet2x.classList.contains('js_bet2x-activeEffect')) {
      lostPoint.textContent ='';
      lostText.textContent = 'BET';
      setTimeout(() => {
        lostPoint.textContent = 500;
      }, 50);
    return;
  } 
    else if(bet5x.classList.contains('js_bet5x-activeEffect')) {
      ...
      lostText.textContent = 'BET';
      setTimeout(() => {
        lostPoint.textContent = 50;
      }, 50);
    }
  } 
  // if(total <= 10000) {
  //   lostText.textContent = 'LOST';
  // }
}

// function switchTiming() {
//   if(total <= 10050 && total >= 10000) {
//     lostText.textContent = 'BET';
//     lostPoint.textContent = 50;
//   }
// }
...
const insertPoint = document.querySelector('.btn-insert');
  insertPoint.addEventListener('click', function () {
    checkOutDept = false; // (R1)
    if(total > 0) return;
    if(currentDept >= 10000) {
      lostPoint.textContent = 0; // (R1)
    }
    total += 10000; 
    currentDept += 10000; 
    totalPoint.textContent = total; 
    ...
    checkOut.textContent = 'CHECK OUT'; // (R1)
...
const bet2x = document.querySelector('.btn-bet2x'); 
  bet2x.addEventListener('click', function () {
    ...
    if(total < 500 && bet5x.classList.contains('js_bet5x-activeEffect')) return; ////// new !!!
    if(total < 100) return; // currentDept === 0 works fine //////////////////////////////////// new !!!
...
const bet5x = document.querySelector('.btn-bet5x');
  bet5x.addEventListener('click', function () {
    if(checkOutDept) return; // (R1)
    if(total < 500 && bet2x.classList.contains('js_bet2x-activeEffect')) return; //////// new !!!
    if(total < 250) return; // currentDept === 0 works fine ///////////////////////////////////// new !!!
    checkOut.textContent = 'CHECK OUT';  // reset check out text //
    checkOut.classList.remove('js_checkOut-win', 'js_checkOut-lost'); // reset message //
    if(activeLight) return;
    if(total <= 0) return;
...
const checkOut = document.querySelector('.check-out');
  checkOut.addEventListener('click', function () {
    if(applyCheckOut) return;
      bet2x.classList.remove('js_bet2x-activeEffect'); // reset bets2x when click check out //
      bet2x.classList.remove('js_bet2x-textColorBright');
      bet5x.classList.remove('js_bet5x-activeEffect'); // reset bets2x when click check out //
      bet5x.classList.remove('js_bet5x-textColorBright');
    if(total > currentDept) {
      checkOut.textContent = `WIN + ${total - currentDept}`;
    } 
    else if(currentDept >= 10000) {
      checkOut.textContent = `LOST - ${currentDept - total}`;
    }
    if(total > currentDept) {      // win & lost colored when check out //
      checkOut.classList.add('js_checkOut-win');
    }
    else if(total < currentDept) { // win & lost colored when check out //
    ...
    if(total > currentDept) return; 
    if(!checkOutDept) { // run only total even currentDept //
...
const spinBtn = document.getElementById('spin');
  spinBtn.addEventListener('click', () => { 
    ...
    if(spinBtn.classList.contains('js_inActive')) return;  // ポイント // dbl click cancel //
        ...
        insertPoint.classList.remove('js_blank') // reset位置はここで正解 // BIG SPIN // ///////////// !!!
        console.log('classList removed -- js_blank');

        playerBet(); // bets point //
        betCounter(); //////////////////////////////// !!! 
        betCount = true; ///////////////// !!! 二度打ち防止
        console.log(total); // console.log //
    panels.forEach(panel => {
      panel.spin();
      btn5x2x_ActiveEffect_TurnOffAuto(); // activateEffect bets5x2x turn off auto //
    });
  });
